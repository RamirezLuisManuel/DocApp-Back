...
class HistorialController {
    ...
    async crearHistorial(req, res) {
        ...
        try {
            ...
            const [usuarios] = await connection.query('SELECT rol FROM usuarios WHERE id = ?', [userId]);
            ...
            const [citas] = await connection.query('SELECT paciente_id, medico_id, fecha_cita FROM citas WHERE id = ?', [historialData.cita_id]);
      ...
      `;
        ...
        `;
            ...
            await connection.query('UPDATE citas SET estado = ? WHERE id = ?', ['completada', historialData.cita_id]);
    ...
    async obtenerHistorialPaciente(req, res) {
      ...
      `;
            const [historiales] = await database_1.default.query(query, [paciente_id]);
    ...
    async obtenerDetalleConsulta(req, res) {
        try {
            const { historial_id } = req.params;
            // Obtener historial
            const [historiales] = await database_1.default.query(`SELECT h.*, CONCAT(m.nombre, ' ', m.apellido) as medico_nombre,
         e.nombre as especialidad
         FROM historial_medico h
         INNER JOIN usuarios m ON h.medico_id = m.id
         INNER JOIN perfiles_medicos pm ON m.id = pm.usuario_id
         ...
         WHERE h.id = ?`, [historial_id]);
            const historialesArray = historiales;
            ...
            // Obtener recetas
            const [recetas] = await database_1.default.query('SELECT * FROM recetas_medicas WHERE historial_id = ?', [historial_id]);
            res.json({
                success: true,
                data: {
                    historial,
    ...
    async obtenerRecetas(req, res) {
        try {
            const { historial_id } = req.params;
            const [recetas] = await database_1.default.query('SELECT * FROM recetas_medicas WHERE historial_id = ? ORDER BY fecha_creacion', [historial_id]);

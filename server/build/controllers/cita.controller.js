...
class CitaController {
    ...
    async obtenerCitas(req, res) {
        try {
            const [citas] = await database_1.default.query('SELECT * FROM citas ORDER BY fecha_cita DESC');
    ...
    async obtenerCitaPorId(req, res) {
        try {
            const { id } = req.params;
            const [citas] = await database_1.default.query('SELECT * FROM citas WHERE id = ?', [id]);
            const citasArray = citas;
    ...
    async crearCita(req, res) {
        try {
            ...
            const [usuarios] = await database_1.default.query('SELECT rol FROM usuarios WHERE id = ?', [userId]);
    ...
    async obtenerMedicos(req, res) {
      ...
      `;
            const [medicos] = await database_1.default.query(query);
    ...
    async cancelarCita(req, res) {
        try {
            ...
            // Actualizar estado
            await database_1.default.query('UPDATE citas SET estado = ?, motivo_cancelacion = ? WHERE id = ?', ['cancelada', motivo_cancelacion || null, id]);
            res.json({
                success: true,
    ...
    async reprogramarCita(req, res) {
        try {
            ...
            await database_1.default.query('UPDATE citas SET fecha_cita = ?, estado = ? WHERE id = ?', [fecha_cita, 'solicitada', id]);
            res.json({
                success: true,
    ...
    `;
            const [citas] = await database_1.default.query(query, [paciente_id]);
            res.json({ success: true, data: citas });
        }
        catch (error) {
            console.error(error);
    ...
    async obtenerCitasMedico(req, res) {
      ...
      ORDER BY c.fecha_cita DESC
    `;
            const [citas] = await database_1.default.query(query, [medico_id]);
            res.json({ success: true, data: citas });
        }
        catch (error) {
            console.error(error);
    ...
    async confirmarCita(req, res) {
        try {
            ...
            await database_1.default.query('UPDATE citas SET estado = ? WHERE id = ?', ['confirmada', id]);
            res.json({
                success: true,
    ...
    async completarCita(req, res) {
        try {
            ...
            await database_1.default.query('UPDATE citas SET estado = ?, notas_medico = ? WHERE id = ?', ['completada', notas_medico || null, id]);
            res.json({
                success: true,
    ...
    // Rechazar cita
    async rechazarCita(req, res) {
        try {
            const { id } = req.params;
            const { motivo_cancelacion } = req.body;
            await database_1.default.query('UPDATE citas SET estado = ?, motivo_cancelacion = ? WHERE id = ?', ['cancelada', motivo_cancelacion || 'Rechazada por el médico', id]);
            res.json({
                success: true,
